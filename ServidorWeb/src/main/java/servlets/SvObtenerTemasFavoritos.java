 ...
 * @author dokgo
 */
@WebServlet(name = "SvObtenerTemasFavoritos", urlPatterns = {"/SvObtenerTemasFavoritos"})
public class SvObtenerTemasFavoritos extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String temasFavoritosJson = mapper.writeValueAsString(temasFavoritos);
            
            // Enviar respuesta
            response.setContentType("application/json");
            response.setCharacterEncoding("UTF-8");
            response.getWriter().write(temasFavoritosJson);
            
        } catch (Exception e) {
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("Error al obtener temas favoritos: " + e.getMessage());
        }
    }
