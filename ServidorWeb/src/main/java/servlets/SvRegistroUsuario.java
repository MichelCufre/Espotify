...
//    iSistema sys = new Fabrica().getSistema(cpu);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!isValidEmail(email)) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Email inválido: debe contener @");
            return;
        }       
        try{
        ...
        try {
            ...
            sys.altaPerfil(nick, nombre, apellido, password, email, fecha.toString(), rutaimg, biografia, sitioWeb, tipo);    
