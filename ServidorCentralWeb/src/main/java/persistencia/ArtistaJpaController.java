...
public class ArtistaJpaController implements Serializable {
    ...
    public void destroyPorNickname(String nickname) throws Exception {
        EntityManager em = null;
        try {
            em = getEntityManager();
            em.getTransaction().begin();

            // Busca al artista por su nickname
            Artista artista = em.createQuery(
                "SELECT a FROM Artista a WHERE a.nickname = :nickname", Artista.class)
                .setParameter("nickname", nickname)
                .getSingleResult();

            // Si el artista no se encuentra, lanza una excepción
            if (artista == null) {
                throw new Exception("El artista con nickname " + nickname + " no existe.");
            }
            ...
            throw new Exception("El artista con nickname " + nickname + " no existe.", enfe);
        } finally {
            if (em != null) {
                em.close();
            }
