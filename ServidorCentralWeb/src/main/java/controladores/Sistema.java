...
public class Sistema implements iSistema{
    ...
    public static Sistema getInstance(ControladorPersistencia cpu){
        if(instancia == null){
            instancia = new Sistema(cpu);
        }
        return instancia;
    }
    
    public Sistema(ControladorPersistencia cpu){
        this.cpu = cpu;
    }
    ...
    //Testing referencia circular creando coleccion en usuario
    public void seguirUsuario(String nickClienteQueSigue, String nickUsuarioASeguir) {
        Cliente clienteQueSigue = ObtenerCliente(nickClienteQueSigue);
        Usuario usuarioASeguir = obtenerUsuario(nickUsuarioASeguir);

        if (usuarioASeguir != null && clienteQueSigue != null) {
            clienteQueSigue.getSiguiendo().add(usuarioASeguir);
            // Lógica de persistencia si es necesario
        } else {
            throw new IllegalArgumentException("Usuario no encontrado");
        }
        cpu.actualizarUsuario(clienteQueSigue);
    }

    public void dejarDeSeguir(String nickClienteQueSigue, String nickUsuarioADejarDeSeguir) {
        Cliente clienteQueSigue = ObtenerCliente(nickClienteQueSigue);
        Usuario usuarioADejarDeSeguir = obtenerUsuario(nickUsuarioADejarDeSeguir);

        if (usuarioADejarDeSeguir != null && clienteQueSigue != null) {
            clienteQueSigue.getSiguiendo().remove(usuarioADejarDeSeguir);
            // Lógica de persistencia si es necesario
        } else {
            throw new IllegalArgumentException("Usuario no encontrado");
        }
...
//    public void seguirUsuario2(String nickClienteQueSigue, String nickUsuarioASeguir) {
//        Cliente clienteQueSigue = ObtenerCliente(nickClienteQueSigue);
//        Usuario usuarioASeguir = obtenerUsuario(nickUsuarioASeguir);
//
//        clienteQueSigue.getUsuariosSiguiendo().add(usuarioASeguir);
//        usuarioASeguir.getSeguidores().add(clienteQueSigue);
//        cpu.actualizarUsuario(clienteQueSigue);
//    }
    
    public void seguirUsuario2(String nickClienteQueSigue, String nickUsuarioASeguir) throws Exception {
        // Validate input
        if (nickClienteQueSigue == null || nickUsuarioASeguir == null) {
        ...
        // Check if client can follow
        if (!clienteQueSigue.permiteSeguir()) {
            throw new Exception("Solo los clientes pueden seguir a otros usuarios");
        }

        // Check if already following
        if (clienteQueSigue.getSiguiendo().contains(usuarioASeguir)) {
        ...
        // Persist changes
        try {
            // Depending on your persistence layer, you might use:
            
            // Or
            cpu.actualizarUsuario(clienteQueSigue);
            cpu.actualizarUsuario(usuarioASeguir);
        } catch (Exception e) {
            // Rollback changes if persistence fails
            clienteQueSigue.getSiguiendo().remove(usuarioASeguir);
            usuarioASeguir.getSeguidores().remove(clienteQueSigue);
            throw new Exception("Error al seguir usuario: " + e.getMessage());
        }
    }
    
    ///album nueva para eliminar generos de album en web service soap
    public void eliminarGenerosDeAlbum(Album album) {
        album.eliminarGeneros();
    } 
    
    
    //////////////////////////////Artistas eliminados Servidorweb////////////////////////////////////////////////////////////////
    
    public void AltaArtistaEliminado(String nickname, String nombre, String apellido, String email, String imagen, String biografia, String sitioWeb){
    
        ArtistaEliminado agregarArtista = new ArtistaEliminado(nickname, nombre, apellido,  email, imagen, biografia, sitioWeb);
        cpu.crearArtistaEliminado(agregarArtista);
        
    }
    
    public void AltaTemaEliminado(String nombre, String duracion, int posicion, String direccionWeb, String archivo){
    
        TemaEliminado agregarTema = new TemaEliminado(nombre, duracion, posicion, direccionWeb, archivo);
        cpu.crearTemaEliminado(agregarTema);
        
    }
    
    public TemaEliminado getTemaEliminado(String nombre){
        TemaEliminado Te = cpu.getTemaEliminado(nombre);
        return Te;
    }
    
    public void AltaAlbumEliminado(String nombre, Integer fechaCreacion, String imagenAlbum){
        
        AlbumEliminado agregarAlbum = new AlbumEliminado(nombre, fechaCreacion, imagenAlbum);
        cpu.crearAlbumEliminado(agregarAlbum);
        
    }
    
    public AlbumEliminado getAlbumEliminado(String nombre){
        AlbumEliminado Ae = cpu.obtenerAlbumEliminadoPorNombre(nombre);
        return Ae;
    }
    
    public ArtistaEliminado getArtistaEliminado(String nombre){
        ArtistaEliminado Ae = cpu.obtenerArtistaEliminado(nombre);
        return Ae;
    }
    
    public void EliminarTemaPorNombre(String nombre){
        cpu.eliminarTemaTest(nombre);  
    }
    
    public void EliminarAlbumPorNombre(String nombre){
        cpu.eliminarAlbumTest(nombre);
    }
    
    public void EliminarArtistaPorNickname(String nickname){
        cpu.EliminarArtista(nickname);
    }
    
    public void EliminarUsuarioTest(String nickname){
        cpu.EliminarUsuarioTest(nickname);
    }
    
    ///////////ServidorcentralconsultaARTISTAELIMINADOS//////////////////////////////
    public DataArtistaEliminado[] getArtistasEliminados() throws UsuarioNoExisteException {
    List<ArtistaEliminado> artistasE = cpu.obtenerTodosLosArtistasEliminados();
    if (!artistasE.isEmpty()) {
        DataArtistaEliminado[] dataArtistasE = new DataArtistaEliminado[artistasE.size()];
        for (int i = 0; i < artistasE.size(); i++) {
            dataArtistasE[i] = artistasE.get(i).devolverData();
        }
        return dataArtistasE;
    } else {
        throw new UsuarioNoExisteException("No existen artistas eliminados");
        }
    }
    
    public List<DataAlbumEliminado> traerAlbumesArtistaEliminado(String artista) {
        ArtistaEliminado art = cpu.obtenerArtistaEliminado(artista);
        List<DataAlbumEliminado> albumesData = new ArrayList<>();

        if (art != null) {
            List<AlbumEliminado> albumes = (List<AlbumEliminado>) cpu.getAlbumesArtEliminado(art);

            for (AlbumEliminado album : albumes) {
                // Extraer datos del álbum
                Long id = album.getId();
                String nombre = album.getNombre();
                Integer fechaCreacion = album.getFechaCreacion();
                String imagenAlbum = album.getImagenAlbum();
                String nombreArtista = art.getNombre(); // Suponiendo que tienes un método getNombre en ArtistaEliminado

                // Obtener temas como objetos DataTemaEliminado
                List<DataTemaEliminado> temas = new ArrayList<>();
                for (TemaEliminado tema : album.getTemas()) {
                    DataTemaEliminado dataTema = new DataTemaEliminado(
                        tema.getNombre(),
                        tema.getDuracion(),
                        tema.getPosicion(),
                    ...
                    dataTema.setId(tema.getId()); // Si necesitas incluir el id del tema
                    temas.add(dataTema);
                }

                // Crear el DataAlbumEliminado
                DataAlbumEliminado dataAlbum = new DataAlbumEliminado(id, nombre, fechaCreacion, imagenAlbum, nombreArtista, temas);
                albumesData.add(dataAlbum);
            }
        }
    ...
    public void altaRegistro(String ip, String url, String userAgent, String os){
        
        RegistroAcceso registro = new RegistroAcceso(ip, url, userAgent, os);
        cpu.crearRegistroAcceso(registro);
    }
    
    public List<DataRegistro> obtenerRegistros(){
        List<RegistroAcceso> registrosAcceso = cpu.obtenerRegistros();
        List<DataRegistro> dataRegistros = new ArrayList<>();
        
        if(registrosAcceso != null && !registrosAcceso.isEmpty()){
            for (RegistroAcceso registro: registrosAcceso){
                int id = registro.getId();
                String ip = registro.getIp();
                String url = registro.getUrl();
                String userAgent = registro.getUserAgent();
                String os = registro.getOs();

                DataRegistro dataRegistro = new DataRegistro(id, ip, url, userAgent, os);

                dataRegistros.add(dataRegistro);
            }
        }
    ...
    public void borrarRegistrosAcceso(){
        try{
        cpu.borrarRegistroAcceso();
        }catch(Exception e){}
    }
    
    public int obtenerTotalRegistros(){
        return cpu.obtenerTotalRegistros();
        
    }
    
    //Actualizar
    public void actualizarAlbumEliminadoSistema(AlbumEliminado AlbumE){
        cpu.actualizarAlbumEliminado(AlbumE);
    }
    
    public void actualizarTemaEliminadoSistema(TemaEliminado TemaE){
        cpu.actualizarTemaEliminado(TemaE);
    }
    
    public void actualizarArtistaEliminado(ArtistaEliminado ArtistaE){
        cpu.actualizarArtistaEliminado(ArtistaE);
    }
    
    
    //////CONSTRUCTOR DE DATATYPE USUARIO
    public DataUsuario DataUsuarioArtistaCompleto(String nick, String nombre, String apellido, String correo, String imagenP, String biografia, String sitioWeb, String tipo){
        return new DataUsuario(nick, nombre, apellido, correo, imagenP, biografia, sitioWeb, tipo);
    }
    
    public DataUsuario DataUsuarioClienteCompleto(String nick, String nombre, String apellido, String correo, String imagenP, String tipo){
        return new DataUsuario(nick, nombre, apellido, correo, imagenP, tipo);
    }
    
    //////CONSTRUCTOR DE DATATYPE LISTA
    public DataLista DataListaRIC(String nombre, String rutaimg, String cliente){
        return new DataLista(nombre, rutaimg, cliente);
    }
    
    //getAllTema para SOAP
    public List<DataTema> getAllTemas(){
        List<Tema> temas = cpu.Listar_Temas();
        List<DataTema> dtTemas = new ArrayList<>();
        
        for (Tema tema : temas){
            DataTema dtTema = tema.getDataTemaX();
            dtTemas.add(dtTema);
...
//        }
    ...
    public List<DataUsuario> getClientes2() throws UsuarioNoExisteException {
    List<Cliente> clientes = cpu.obtenerTodosLosClientes();
    if (!clientes.isEmpty()) {
        List<DataUsuario> dataClientes = new ArrayList<>();
        for (Cliente cliente : clientes) {
            dataClientes.add(cliente.devolverData());
        }
        return dataClientes;
    } else {
        throw new UsuarioNoExisteException("No existen clientes registrados");
...
//    }
    ...
    Cliente cliente_elegido = cpu.obtenerCliente(cliente_seleccionado);
    List<Usuario> usuarios = cliente_elegido.getUsuariosSiguiendo();
...
//    public UsuariosSeguidosResponse UsuarioSeguidosWeb(String cliente_seleccionado) {
//    Cliente cliente_elegido = cpu.obtenerCliente(cliente_seleccionado);
//    ArrayList<Usuario> usuarios = cliente_elegido.getUsuariosSiguiendo();
//
//    ArrayList<DataUsuario> clientesSeguidos = new ArrayList<>();
...
//
//        if (usuario instanceof Cliente) {
//            clientesSeguidos.add(dataUsuario);
//        } else if (usuario instanceof Artista) {
//            artistasSeguidos.add(dataUsuario);
//        }
//    }
//
//    UsuariosSeguidosResponse response = new UsuariosSeguidosResponse();
//    response.setClientes(clientesSeguidos);
//    response.setArtistas(artistasSeguidos);
//
//    return response;
//}
    
    public UsuariosSeguidosResponse UsuarioSeguidosWeb(String cliente_seleccionado) {
        Cliente cliente_elegido = ObtenerCliente(cliente_seleccionado);
        List<Usuario> usuarios = cliente_elegido.getUsuariosSiguiendo();
        ...
        for (Usuario usuario : usuarios) {
            ...
            if (usuario instanceof Cliente) {
                response.getClientes().add(dataUsuario);
            } else if (usuario instanceof Artista) {
                response.getArtistas().add(dataUsuario);
            }
        }
...
//    }
   ...
   //Listas de Reproduccion
    ...
    public boolean tipoLista(int id){
        if(cpu.findLista(id) instanceof ListaParticular){
            return true;
        } else{ return false; }
    }
   ...
   public ArrayList Nicknames_De_Clientes(){
    ...
    public List<String> Listar_Usuarios_A_Seguir(String clienteSeleccionado) {
        List<Usuario> todosLosUsuarios = cpu.getUsuarios();
        Cliente clienteSeguidor = cpu.obtenerCliente(clienteSeleccionado);

        List<Usuario> usuariosSeguidos = clienteSeguidor.getUsuariosSiguiendo();
        List<String> nicknamesSeguidos = new ArrayList<>();

        // Extrae los nicknames de los usuarios que ya está siguiendo
        for (Usuario usuarioSeguido : usuariosSeguidos) {
            nicknamesSeguidos.add(usuarioSeguido.getNickname());
        }

        // Copia todos los usuarios
        List<Usuario> usuariosFiltrados = new ArrayList<>(todosLosUsuarios);
        List<String> nicknamesFiltrados = new ArrayList<>();

        // Extrae los nicknames de todos los usuarios
        for (Usuario usuario : usuariosFiltrados) {
            nicknamesFiltrados.add(usuario.getNickname());
        }

        // Elimina de la lista los usuarios que ya está siguiendo
        nicknamesFiltrados.removeAll(nicknamesSeguidos);

        return nicknamesFiltrados;
    }
    ...
    public List<String> Listar_Usuarios_A_DEJAR_DE_Seguir(String clienteSeleccionado) {
        Cliente clienteElegido = cpu.obtenerCliente(clienteSeleccionado);
        List<Usuario> usuarios = clienteElegido.getUsuariosSiguiendo();
        List<String> usuariosString = new ArrayList<>();

        for (Usuario usuario : usuarios) {
            usuariosString.add(usuario.getNickname());
        }
    ...
    public List<DataLista> traerListasPublicasTodas(){
        List<DataLista> dtl= new ArrayList<>();
        for(DataLista s:traerListasParticularesPublicas()){
            dtl.add(s);
        }
        return dtl;
    }
    ...
    public List<DataSub> listarSubsPendientes(){
        List<DataSub> dts = new ArrayList<>();
        for(Usuario c:cpu.traerUsuariosClientes()){
            if(c instanceof Cliente){
                if(((Cliente) c).getSub()!=null){
                    if(((Cliente) c).getSub().getEstado()=="Pendiente"){
                        dts.add(new DataSub(((Cliente) c).getSub()));
                    }
                }
            }
        }
    ...
    public List<String> listarUsuarioSubsPendientes(){
        ...
        for(Cliente c: cpu.obtenerTodosLosClientes()){
            if(c.getSub()!=null){
                if(c.getSub().getEstado().equals("Pendiente")){
    ...
    public DataSub darSubUsuario(String nick){
        Cliente cli = cpu.obtenerCliente(nick);
        DataSub dt = new DataSub(cli.getSub());
        return dt;
    }
...
//        } else { 
//            try {
//                cpu.actualizarSub(sub);
//            } catch (Exception ex) {
//                Logger.getLogger(Sistema.class.getName()).log(Level.SEVERE, null, ex);
//            }
//        }
//        cli.setSub(sub);
//        cpu.actualizarUsuario(cli);
//        
//        try {
//            EmailService.enviarNotificacion(sub);
//        } catch (MessagingException ex) {
//            Logger.getLogger(Sistema.class.getName()).log(Level.SEVERE, 
//                "Error al enviar la notificación de suscripción", ex);
//        }
//    }
        ...
        Subscripcion sub = new Subscripcion();
        sub.setNickname(nick);
        sub.setTipo(tipo);
        
        // Crear o actualizar suscripción
        if (cli.getSub() == null) {
            cpu.crearSub(sub); // Crear nueva suscripción
        } else {
            sub.setId(cli.getSub().getId()); // Usar el ID existente
            try {
                cpu.actualizarSub(sub); // Actualizar suscripción existente
            } catch (Exception ex) {
                Logger.getLogger(Sistema.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        ...
        cli.setSub(sub); // Asociar cliente a la suscripción
        cpu.actualizarUsuario(cli); // Actualizar cliente en la BD
...
//    // Artistas
    ...
    //prueba
    altaPerfil("cliente1", "cliente1", null, "cliente1", "cliente1@gmail.com", LocalDate.parse("1971-12-31"), "eleven_cliente.png", null, null, CLIENTE);
    altaPerfil("cliente2", "cliente2", null, "cliente2", "cliente2@gmail.com", LocalDate.parse("1971-12-31"), "eleven_cliente.png", null, null, CLIENTE);
    altaPerfil("artista1", "artista1", "artista1", "artista1", "artista1@hotmail.com", LocalDate.parse("1980-10-31"), "nicoleneuman_artista.jpg", null, null, ARTISTA);
    
        try {
            seguirUsuario2("cliente1","cliente2");
        } catch (Exception ex) {
            Logger.getLogger(Sistema.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//    AgregarListaFavoritaCliente("el_padrino", lista);
    ...
    altaTema("YMCA", "4:28", 1, "-", "Y.M.C.A.mp3");
    altaTema("Macho Man", "3:28", 2, "-", "Macho Man.mp3");
    altaTema("In the Navy", "3:13", 3, "-", "In The Navy.mp3");
    altaAlbum();

    cargarDatosAlbum("Violator", "dmode", 1990, null);
    addGeneroAlbum("Electropop");
    altaTema("Personal Jesus", "4:56", 1, "-", "Personal Jesus.mp3");
    altaTema("Enjoy The Silence", "4:21", 2, "-", "Enjoy the Silence.mp3");
    altaAlbum();
    ...
    altaTema("Time After Time", "5:12", 2, "-", "Time after time.mp3");
    altaAlbum();
    ...
    addGeneroAlbum("Rock Clásico");
    addGeneroAlbum("Pop Clásico");
    altaTema("It’s Not Unusual", "2:00", 1, "-", "Its Not Unusual.mp3");
    altaAlbum();
   ...
   public boolean Encontrar_Lista_Particular(String nombre_cliente,String nombre_lista){
           ...
           for(ListaParticular lp:listas){
               if(lp.getNombre().equals(nombre_lista)){
                   if(lp.getCliente().equals(nombre_cliente)){
                       return true;
                   }
               }
           }
   ...
   public List<DataAlbum> getAlbumsByArtista2(String artista) throws AlbumNoExisteException {
    List<Album> albums = cpu.obtenerAlbumPorArtista(artista);
    if (!albums.isEmpty()) {
        List<DataAlbum> dataAlbums = new ArrayList<>();
        for (Album album : albums) {
            DataAlbum data = album.devolverData();
            dataAlbums.add(data);
