...
//                    }
                    ...
                    // Seguidores
                    List<DataUsuario> seguidores = sys.getSeguidores(nickname); // Ahora usa DataUsuario

                    DefaultListModel<String> modeloSeguidores = new DefaultListModel<>();
                    for (DataUsuario seguidor : seguidores) {
                        modeloSeguidores.addElement(seguidor.getNick());  // Utiliza el método getNick() de DataUsuario
                    }
                    list_seguidores.setModel(modeloSeguidores);

                    // Usuarios Seguidos
                    Map<String, ArrayList<DataUsuario>> usuariosSeguidos = sys.UsuarioSeguidos(nickname); // Cambiado a DataUsuario

                    // Clientes seguidos
                    DefaultListModel<String> modeloClientesSeguidos = new DefaultListModel<>();
                    ArrayList<DataUsuario> clientesSeguidos = usuariosSeguidos.get("clientes"); // Cambiado a ArrayList<DataUsuario>

                    for (DataUsuario cliente : clientesSeguidos) {
                        modeloClientesSeguidos.addElement(cliente.getNick());  // Utiliza el método getNick() de DataUsuario
                    }
                    jlist_cliente_seguido.setModel(modeloClientesSeguidos);

                    // Artistas seguidos
                    DefaultListModel<String> modeloArtistasSeguidos = new DefaultListModel<>();
                    ArrayList<DataUsuario> artistasSeguidos = usuariosSeguidos.get("artistas"); // Cambiado a ArrayList<DataUsuario>

                    for (DataUsuario artista : artistasSeguidos) {
                        modeloArtistasSeguidos.addElement(artista.getNick());  // Utiliza el método getNick() de DataUsuario
                    }
